...
/** Describe a document for the library grid, as of `at` (epoch ms). */
export function panelEntryFor(path: string, project: Project, at: number): PanelEntry {
...
export function recordPanelOpened(library: PanelLibrary, entry: PanelEntry): PanelLibrary {
  ...
  return { version: PANEL_LIBRARY_VERSION, entries: [entry, ...rest].slice(0, MAX_LIBRARY_ENTRIES) }
}
