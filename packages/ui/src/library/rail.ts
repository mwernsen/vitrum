 ...
 * cross-document destinations, not just a grid. Only destinations that exist or are actively
 * planned earn a row: "Panels" is live, "Glass library" is the next destination (F-063) and renders
 * disabled until it lands — the repo's established convention for unbuilt surfaces (`shell/dock.ts`,
 * `shell/viewmode.ts`): never silently absent, never fake-clickable.
 *
 ...
 * placeholder is noise. They return when a spec claims them.
 */
...
export interface RailItem {
  ...
  /** Whether the destination exists. Only "Panels" does today. */
...
export const RAIL_ITEMS: readonly RailItem[] = [
  { id: 'panels', label: 'Panels', icon: LayoutGrid, live: true },
  {
    ...
    icon: Droplet,
    live: false,
    note: 'Your glass catalog opens here soon (F-063)',
  },
]
